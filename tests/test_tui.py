...
async def _goto_profile(pilot) -> None:
    """Walk the section strip to PROFILE with the real binding.

    Presses `alt+right` until it arrives rather than a fixed number of times.
    Four tests counted the presses, so adding the EXTRACTIONS section between
    UNRESOLVED and PROFILE silently landed them one tab short -- they then
    asserted against widgets belonging to a section that was not showing, and
    failed somewhere unrelated to what they were testing.
    """
    ...
    for _ in range(len(SECTION_FOR_TAB)):
        ...
        if switcher.current == SEC_PROFILE:
            break
...
def _accounts_rows(app) -> dict[str, list[str]]:
    """The ACCOUNTS table as {site name: cells}.

    Keyed off the site cell rather than the first one -- column 0 is the status
    glyph, which is identical on every row.
    """
    from textual.widgets import DataTable

    from sherlock_project.tui.results_pane import SEC_ACCOUNTS

    table = app.query_one(f"#{SEC_ACCOUNTS}", DataTable)
